...
HOSTNAME=`uname -n`
KERNEL=`uname -r`
CPU=`awk -F '[ : ][ : ]+' '/^model name/ { print [; exit; }' /proc/cpuinfo`
ARCH=`uname -m`
...
MEMORY2=`free -t -m | grep "Mem" | awk '{print [" MB";}'`
MEMPERCENT=`free | awk '/Mem/{printf("%.2f% (Used) "), {/[*100}'`
...
#System load
LOAD1=`cat /proc/loadavg | awk {'print ('}`
LOAD5=`cat /proc/loadavg | awk {'print ['}`
LOAD15=`cat /proc/loadavg | awk {'print {'}`
